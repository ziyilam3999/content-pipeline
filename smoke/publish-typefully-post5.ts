/**
 * Publish-to-Typefully smoke for POST #5 ("the 3-role development model — nobody grades their own
 * homework") — #841. Assembles the Post #5 launch DRAFT and either prints it (DRY-RUN, default) or
 * actually creates it (LIVE, gated — orchestrator only). This is a SIBLING of
 * smoke/publish-typefully-post3.ts (Post #3): it REUSES the SAME assembly helpers (`PromoThread`,
 * `PlatformPrimaryPost`) and the SAME consolidated #797 fidelity gate (`assertPostAssemblyFidelity`)
 * from publish/promoMedia — it does NOT fork the gate logic. The only things that differ from Post #3
 * are the COPY, the VIDEO bundle, and the CARDS:
 *
 *   - Post #3 = the forge-harness story ("8 primitives, only 1 ever talks to the model").
 *   - Post #5 = the 3-role development model ("four AI subagents, nobody grades their own homework");
 *     a 4-TWEET X thread + a Threads post, post5-demo-9x16 hero, card-post5-{A,B,C}.png body cards.
 *
 * THE PRINCIPLE (#792, baked in publish/promoMedia.ts): EVERY platform's primary worded post LEADS
 * WITH VIDEO and every worded unit ALSO carries its card-over-art infographic. Post #5 realization:
 *   - X (no image+video mixing in one tweet): SPLIT into a video HOOK tweet (post5-demo-9x16.mp4, the
 *     full-bleed 9:16 phone HERO — #794) + 3 card body tweets (card-post5-{A,B,C}.png). 4 tweets
 *     total. Modeled as a PromoThread.
 *   - Threads (mixed-media carousel): a SINGLE post whose media is ORDERED [post5-demo-9x16.mp4
 *     (HERO/lead, full-bleed 9:16), card-post5-A.png (second)]. VIDEO LEADS, CARD PRESENT. Modeled as
 ...
 * placeholders like `<upload:post5-demo-9x16.mp4>`), assert every media file exists + print sizes,
 * run the gate, make ZERO network calls (no Typefully client is even constructed), and print:
 *     FIDELITY: PASS
 *     PUBLISH-TYPEFULLY-POST5: mode=dry-run posts=x:4,threads:1 media=6
 *
 ...
 * approved render frozen in publish/manifests/three-role-model-post5.publish-manifest.json. Order of
 * operations: operator APPROVES the renders → freeze the receipt → publish. Re-render+re-approve ⇒
 * RE-FREEZE first.
 *
 * Run:
 *   npm run publish:freeze-manifest -- three-role-model-post5   (freeze approved hashes — run AFTER approval)
 *   npm run smoke:publish-typefully-post5                       (dry-run, zero network)
 *   npm run smoke:publish-typefully-post5:live                  (LIVE — orchestrator only, post operator go)
 *
...
/**
 * Post #5's renders + copy were produced in THIS worktree's gitignored out/ working dirs (where the
 * #841 art+video+card scaffolding leg wrote them), NOT in the primary clone. Resolve them from the
 * worktree repo root by default; override with $CONTENT_PIPELINE_PRIMARY for portability (mirrors the
 * path-override env var used by smoke/publish-typefully-post3.ts).
 */
const ROOT = process.env.CONTENT_PIPELINE_PRIMARY ?? path.resolve(__dirname, "..");

const POST5_COPY_JSON = path.join(ROOT, "out", "copy", "three-role-model-post-content.json");
// Cards live under out/image/ for Post #5 (the #841 leg rendered them there); the demo videos live in
// their own multi-aspect review dir (mirrors Post #3's demo-post3 dir, but for the post5 bundle).
const IMAGE_DIR = path.join(ROOT, "out", "image");
const DEMO_DIR = path.join(ROOT, "out", "review", "lfah", "demo-post5", "multi-aspect");
...
const DEMO_HERO = path.join(DEMO_DIR, `post5-demo-${HERO_ASPECT_TAG}.mp4`); // 9:16 hero — leads X + Threads
const CARD_POST5 = (letter: "A" | "B" | "C") => path.join(IMAGE_DIR, `card-post5-${letter}.png`);

const SOCIAL_SET_ID = process.env.TYPEFULLY_SOCIAL_SET_ID ?? "312308";
const DRAFT_TITLE = "3-role development model launch — Post 5";

/**
 * Fallback Post #5 copy (used only if the gitignored runtime json is absent, e.g. in CI) so the
 * dry-run still assembles + asserts. Mirrors out/copy/three-role-model-post-content.json VERBATIM —
 * structural counts only (workflow/methodology, no efficacy numbers), brand-clean. x_thread is 4
 * strings (hook + 3 body), threads_post is the single Threads post copy.
 */
const FALLBACK_X_THREAD: string[] = [
  "Most AI coding setups use one agent for everything — it plans the work, does it, then decides it's done.\n\nIt grades its own homework. And when it's wrong, it's confidently wrong.\n\nThe 3-role model splits that into 4 subagents where nobody checks their own work. 🧵",
  "Four jobs, four separate AI subagents:\n\nplanner → what to do\nplan-review → vet the plan\nexecutor → do it\nexecution-review → vet the result\n\nThe orchestrator just coordinates. The reviewer is never the one who wrote the thing.",
  "Two knobs pick the shape per task:\n\n• executor: a test-loop, one subagent, several in parallel, or inline\n• evaluator: a real passing test, an independent reviewer, or both\n\nWhenever a real test exists, it beats an opinion.",
  'Best part: it\'s mechanically enforced.\n\nHooks + a forgery-resistant ledger record which roles really ran, bound to real transcripts — so "we followed the process" is provable, not claimed.\n\nPublic, MIT, early → github.com/ziyilam3999/three-role-model',
];

const FALLBACK_THREADS_TEXT = `Most AI coding setups use one agent for everything — it plans the work, does it, then decides it's done. It grades its own homework.

The 3-role model splits that into 4 subagents: planner → plan-review → executor → execution-review. Nobody reviews their own work.

Two knobs per task: how the executor runs, and how it's checked — a real test beats an opinion. And it's enforced: a ledger records which roles really ran.

Public, MIT, early.
github.com/ziyilam3999/three-role-model`;
...
/**
 * Read the Post #5 copy from the gitignored runtime json, falling back to the inline VERBATIM copy
 ...
 */
function readPost5Copy(): Post5Copy {
  if (!fs.existsSync(POST5_COPY_JSON)) {
    console.log(`(post-5 copy json not found at ${POST5_COPY_JSON} — using inline fallback copy)`);
    return { xThread: FALLBACK_X_THREAD, threadsText: FALLBACK_THREADS_TEXT };
  }
  const raw = fs.readFileSync(POST5_COPY_JSON, "utf8");
  ...
  if (!Array.isArray(arr) || arr.length !== 4 || !arr.every((s) => typeof s === "string")) {
    throw new Error(
      `expected x_thread to be 4 strings in ${POST5_COPY_JSON}, got ${JSON.stringify(arr)?.slice(0, 120)}`,
    );
  }
  const threadsText = json.threads_post;
  if (typeof threadsText !== "string" || threadsText.trim().length === 0) {
    throw new Error(`expected a non-empty threads_post string in ${POST5_COPY_JSON}`);
  }
...
function assertFile(label: string, p: string): number {
  if (!fs.existsSync(p)) {
    throw new Error(
      `SMOKE FAIL: missing ${label} at ${p} (render the Post #5 launch assets first: ` +
        `post5 demo videos under out/review/lfah/demo-post5/multi-aspect/, cards card-post5-{A,B,C}.png ` +
        `under out/image/)`,
    );
  }
...
/**
 * The per-tweet media plan for the Post #5 X thread. `path` is the file PATH; `kind` distinguishes
...
function xThreadSlots(): MediaSlot[] {
  return [
    ...
    { label: "X tweet 2", path: CARD_POST5("A"), kind: "card-over-art" },
    { label: "X tweet 3", path: CARD_POST5("B"), kind: "card-over-art" },
    { label: "X tweet 4", path: CARD_POST5("C"), kind: "card-over-art" },
  ];
}

/**
 * Build the PromoThread for the Post #5 X thread from the slots + the thread text. This is what the
 ...
 * post leads with the full-bleed 9:16 HERO video (#794) then carries card-post5-A. Order is
...
const THREADS_ORDERED_MEDIA: { path: string; kind: "video" | "card-over-art" }[] = [
  ...
  { path: CARD_POST5("A"), kind: "card-over-art" }, // second — the headline infographic card
];
...
async function main() {
  ...
  const { xThread, threadsText } = readPost5Copy();
  const slots = xThreadSlots();

  console.log("POST #5 (the 3-role development model — nobody grades their own homework) — publish assembly\n");
  ...
  // recovery hashes only the hero video + card-post5-A, the assets it actually uploads), resolved from
  // the POST_ASSETS SSOT, and asserts each sha256 matches the operator-approved render frozen in
  // publish/manifests/three-role-model-post5.publish-manifest.json. Freeze AFTER operator approval:
  // `npm run publish:freeze-manifest -- three-role-model-post5`.
  const provenanceAssets: PublishAsset[] = POST_ASSETS["three-role-model-post5"].assets
    .filter((a) => usedBasenames.has(a.basename))
    .map((a) => ({
      role: a.role,
      path: path.join(a.role === "hero-video" ? DEMO_DIR : IMAGE_DIR, a.basename),
    }));
  assertPublishAssetsMatchManifest(provenanceAssets, loadManifest("three-role-model-post5"));
  console.log(
    `PROVENANCE: PASS — ${provenanceAssets.length} assets match the three-role-model-post5 approved manifest (#810)`,
  );
  ...
  console.log(
    `\nFIDELITY: PASS — Post #5 layout passes assertPostAssemblyFidelity over [${platforms.join(", ")}] ` +
  ...
  // ── POST AUTO-ARCHIVE (both modes, non-fatal). Post #5 is assembled + passed the fidelity gate,
  ...
  const archived = safeArchivePostAll(
    buildArchiveRecord("three-role-model-post5", { primaryRoot: ROOT }),
  );
  if (archived)
    console.log(
      `ARCHIVE: three-role-model-post5 copy+metadata saved → ${archived.external.archiveDir} (+ in-repo ${archived.inRepo.archiveDir})`,
    );
  ...
  if (!live) {
    ...
    console.log(
      `\nPUBLISH-TYPEFULLY-POST5: mode=dry-run posts=x:${xCount},threads:${tCount} media=${mediaCount}`,
    );
    process.exit(0);
  }
  ...
  requireEyeballAck(DEMO_HERO, { label: "three-role-model-post5 hero video (pre-publish)" });
  ...
  const tCount = body.platforms.threads?.posts.length ?? 0;
  console.log(
    `\nPUBLISH-TYPEFULLY-POST5: mode=live draft_id=${res.id} status=${res.status} ` +
      `posts=x:${xCount},threads:${tCount} media=${xMediaPaths.length + threadsMediaPaths.length}`,
  );

  // ── LIVE URL WRITEBACK (non-fatal). Post #5 was NOT yet live; once published, MERGE the publish
  ...
  const liveArchived = safeArchivePostAll(
    buildArchiveRecord("three-role-model-post5", {
      primaryRoot: ROOT,
  ...
  if (liveArchived) {
    console.log(
      `ARCHIVE: three-role-model-post5 publish state written back → ${liveArchived.external.metaPath} (+ in-repo ${liveArchived.inRepo.metaPath})`,
    );
  }
  process.exit(0);
}
