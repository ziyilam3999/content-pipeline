/**
 * Post #5 three-role-model demo — committed producer for ALL 3 aspects (9:16 HERO / 1:1 / 4:5), each
 * frame-filling, over the SHARED dimmed ANIMATED card art. The Post #3 twin of
 * `smoke/post3-demo-multi-aspect.ts`, rendering the `post5-demo` composition via
 * `renderPost5DemoVideo` (NOT the shared `adapters/video.ts`, which a concurrent worktree edits).
 *
 * BINDS THE SHARED ART (the whole point — feedback_demo_video_must_bind_shared_art_background_not_silent_solid):
 * the video background is the SAME `_art-base-three-role-model-post5.png` the cards are composed over.
 * This smoke HARD-FAILS if that art file is missing — it NEVER silently renders a solid background.
 * Generate the art FIRST (`LAUNCH_CARD_PAID=1 npx tsx smoke/launch-card-post5.ts`), THEN this.
 *
 * Reads the voiced alignment bundle from `smoke/post5-demo-narrated.ts` (one paid synth, reused for
 ...
 *
 * Run: `npx tsx smoke/post5-demo-multi-aspect.ts`
 */

import * as fs from "fs";
import * as path from "path";

import { renderPost5DemoVideo } from "../adapters/video-post5";
...
const BUNDLE_PATH = path.join("out", "review", "lfah", "demo-post5", "scene-sync-check.json");

/** The SHARED card art that MUST back the video (bound, not solid) — the deliverable art path. */
const ART_BASE = path.join("out", "image", "_art-base-three-role-model-post5.png");
...
function loadBundle(): NarrationBundle {
  if (!fs.existsSync(BUNDLE_PATH)) {
    throw new Error(
      `SMOKE FAIL: bundle ${BUNDLE_PATH} not found — run smoke/post5-demo-narrated.ts (POST5_DEMO_PAID=1) FIRST.`,
    );
  }
  ...
  if (!script) throw new Error(`SMOKE FAIL: bundle ${BUNDLE_PATH} has no "script" — re-run post5-demo-narrated.`);
...
async function main() {
  ...
  if (!fs.existsSync(ART_BASE)) {
    console.error(
      `SMOKE FAIL: shared art base ${ART_BASE} is MISSING. The three-role-model demo video MUST render ` +
        `over the SAME nano-banana art the cards use — refusing to ship a solid-background video. ` +
        `Generate it first: LAUNCH_CARD_PAID=1 npx tsx smoke/launch-card-post5.ts`,
    );
    process.exit(1);
  }
  ...
  const outDir = path.join(process.cwd(), "out", "review", "lfah", "demo-post5", "multi-aspect");
  ...
  console.log(
    `\n=== Post #5 multi-aspect demo (VOICED) — audio=${path.basename(bundle.audioPath)} ` +
  ...
  assertDemoDurationInWindow(expectedDurationSec, { label: "voiced three-role-model demo" });
  console.log(
  ...
  const spec = threeRoleModelSpec();
  const results: { aspect: string; file: string; bytes: number }[] = [];

  for (const aspectName of ASPECTS) {
    const t0 = Date.now();
    const fileName = `post5-demo-${aspectName.replace(":", "x")}.mp4`;
    const file = await renderPost5DemoVideo(spec, {
      aspectName,
      outDir,
      fileName,
    ...
    console.log(`POST5-DEMO-PATH: aspect=${aspectName} file="${file}" bytes=${bytes} render=${((Date.now() - t0) / 1000).toFixed(1)}s`);
  ...
  console.log(`\nSMOKE PASS: ${results.length}/3 three-role-model aspects rendered (voiced, shared-art-bound, provenance-guarded).`);
  for (const r of results) console.log(`ARTIFACT: ${r.file} (${r.bytes} bytes)`);
  process.exit(0);
}

main().catch((err) => {
  console.error("post5-demo-multi-aspect FAIL (threw):", err instanceof Error ? err.message : String(err));
  process.exit(1);
});
