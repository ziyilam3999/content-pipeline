/**
 * Post #5 — three-role-model demo video adapter (composition id="post5-demo").
 *
 * A SEPARATE adapter from `adapters/video.ts` (which a concurrent worktree is editing) — it renders
 * the Post #5 composition (`remotion/post5-index.tsx`) and is wired by the post5 smokes. It SHARES
 * every render mechanic with `renderPost3DemoVideo` by importing the same pure modules: the audio↔
...
export interface RenderPost5Opts {
  ...
  /** SHARED card art (`_art-base-three-role-model-post5.png`) embedded as the dimmed animated bg. */
...
/**
 * Bundle the post5 Remotion entry and render one composition to an MP4, retrying the select+render on
...
/**
 * Render the Post #5 three-role-model demo MP4 and return its absolute path.
 */
export async function renderPost5DemoVideo(spec: ContentSpec, opts?: RenderPost5Opts): Promise<string> {
  const fps = opts?.fps ?? 30;
  const timeline = buildPost5Timeline(spec, {
  ...
  if (opts?.audioPath && !hasScript) {
    throw new Error(
      "#775 parity: a voiced post5-demo render (audioPath set) must carry captions — pass opts.script (the spoken narration).",
    );
  }
  ...
  const inputProps = {
    ...
    knobs: timeline.knobs,
    enforced: timeline.enforced,
    cta: timeline.cta,
  ...
  const outPath = path.join(outDir, opts?.fileName ?? `post5-demo-${aspectName.replace(":", "x")}.mp4`);

  const entryPoint = path.join(__dirname, "..", "remotion", "post5-index.tsx");
